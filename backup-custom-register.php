        ...
        <v-layout justify-center align-center>
          <v-flex xs12 sm8 md6>
						...
						<div id="content">
							...
							<div id="inner-content" class="row-cont wrap clearfix">
						<?php
						/**
						...
						function show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
							...
							// Blog name ?>
								<div class="field-wrap clearfix text-wrap label-left col3-wrap">
						<?php
							...
							if ( !is_subdomain_install() )
								...
								echo '<input name="blogname" type="text" id="blogname" value="'.esc_attr($blogname).'" maxlength="60" class="col3" /><span class="suffix_address">.' . ( $site_domain = preg_replace( '|^www\.|', '', $current_site->domain ) ) . '</span><br />';
							...
							<?php if ( $errmsg = $errors->get_error_message('blog_title') ) { ?>
								<p class="error"><?php echo $errmsg ?></p>
							<?php }
							echo '<input name="blog_title" type="text" id="blog_title" value="'.esc_attr($blog_title).'" class="col3" />';
							?>
							</div>

							<div id="privacy" class="clearfix">

									<div class="clearfix field-wrap list-radio-wrap label-left col1-wrap yes-no-wrap radio-wrap">
						        <label for="blog_public_on"><?php _e('Allow search engines to index this site:') ?></label>
						        <ul class="clearfix">
						          <li>
						...
						<div class="field-wrap clearfix text-wrap label-left col3-wrap">
							<!-- <v-text-field v-model="username" label="Username" name="user_name" type="text" id="user_name"/> -->
						<?php
							// User name
							...
							<?php if ( $errmsg = $errors->get_error_message('user_email') ) { ?>
								<p class="error"><?php echo $errmsg ?></p>
							<?php } ?>
							<input name="user_email" type="text" class="col3" id="user_email" value="<?php  echo esc_attr($user_email) ?>" maxlength="200" />
						  <?php echo ('<hr class="clear">'); ?>
							<?php _e('<span class="rules">We send your registration email to this address.<br />(Double-check your email address before continuing.)</span>') ?>
							</div>
						...
						function signup_another_blog( $blogname = '', $blog_title = '', $errors = '' ) {
							...
							?>
							<p style="text-align:center;"><?php printf( __( 'Welcome back, %s. By filling out the form below, you can add another site to your account. <br />There is no limit to the number of sites you can have, so create to your heart’s content, but write responsibly!' ), $current_user->display_name ) ?></p>
							<div class="alreadyMember">
								<?php
								$blogs = get_blogs_of_user($current_user->ID);
								if ( !empty($blogs) ) { ?>

										<h3 id="member-of-label"><?php _e( 'Sites you are already a <strong>member of</strong>' ) ?></h3>
										<ul id="member-of">
							...
							<h3><?php _e( 'Your new website <strong>informations</strong>' ) ?></h3>
							<p class="center-align"><?php _e( 'If you’re not going to use a great site domain, leave it for a new user. Now have at it!' ) ?></p>
							<form id="setupform" method="post" action="/register/">
								...
								<p class="submit"><input type="submit" name="submit" class="submit btn" value="<?php esc_attr_e( 'Create Site' ) ?>" /></p>
							</form>
							<?php
						}
